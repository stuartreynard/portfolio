...
async function sendToSlack(config){
    ...
    const payload = {
        username: 'Baltimore Convention Center',
        text: config.url,
    };
...
async function getAvailability(config){
    ...
    for(const dateString of config.dateRange){
        ...
        timeout = Math.round(Math.random()*(12000-7500))+7500;
...
async function main(){
    ...
    config.slackWebhook = '';
    config.url =
'https://signupandschedule.umm.edu/mychart/SignUpAndSchedule/EmbeddedSchedule?id=RES^84003204&VT=22759';
